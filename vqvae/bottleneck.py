...
import vqvae.utils.dist_adapter as dist
import pdb
...
class BottleneckBlock(nn.Module):
    ...
    def quantise(self, x):
        ...
        # (lenght,512)
        k_w = self.k.t()
        # print('bottle k_w',k_w.size())
        # print('bottle x',x.size())  
        # pdb.set_trace()
    ...
    def decode_latent(self, x_l):
        
        N, width, T = x_l.shape
        
        # x_l, prenorm = self.preprocess(x_l)
        x_l = x_l.permute(0, 2, 1).contiguous()
        
        x_l, fit=self.quantise(x_l)
        x_l = x_l.view(N, T)
        ...
        # print('decode laten xd',x_d.shape)
        # print('decode laten xd',x_d[0].size())
        return (x_d, x_l)
    ...
    def forward(self, x, update_k=True):
        ...
        x_d = self.dequantise(x_l)
        print('dequantise',x_d.size())
        

        # Update embeddings
        if update_k:
            update_metrics = self.update_k(x, x_l)
            print('update k metrics!')
        else:
            update_metrics = {}
...
class Bottleneck(nn.Module):
    ...
    def decode_latent(self,zs,start_level=0, end_level=None):
        # if end_level is None:
        #     end_level = self.levels
        # print('botttle neck zs',zs[0].size())
        # print('botttle neck len zs',len(zs))
        # xs_quantised=[]
        # xs_code=[]
        # for (level_block, z) in zip(self.level_blocks[start_level:end_level], zs):
        #     xs=level_block.decode_latent(z)
        #     xs_quantised.append(xs[0])
        #     xs_code.append(xs[1])
        xs_code = self.encode(zs)
        xs_quantised = self.decode(xs_code)
        # print('xs_quantised',xs_quantised[0].size())
        # print('xs_code',xs_code[0].size())
        # pdb.set_trace()
        
        return xs_quantised, xs_code

    def codebook(self):
        codebook = []
        for level_blocks in self.level_blocks:
            codebook.append(level_blocks.return_k())
