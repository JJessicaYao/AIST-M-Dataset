...
# for coco17
# smpl_up = [0, 1, 2, 3, 4, 5, 6, 14]  #8
# smpl_down = [7, 8, 9, 10, 11, 12, 13] #7
...
class SepVQVAE(nn.Module):
    ...
    def quantise_latent(self,zs):
        # pdb.set_trace()
        b,c,t=zs.shape
        zup = zs[:,:,:t//2]
        zdown = zs[:,:,t//2:]
        xup = self.vqvae_up.quantise_latent(zup)        
        xdown = self.vqvae_down.quantise_latent(zdown)
        # pdb.set_trace()
        x_quantised = torch.cat([xup[0],xdown[0]],dim=-1)
        return x_quantised
    ...
    def decode_latent(self, zs, start_level=0, end_level=None, bs_chunks=1):
        ...
        # zup=[zup[0].reshape(1,-1,512)]
        # zup=[zup[0].reshape(1,-1,512)]
        # print('sep vqvae zup',zup[0].size())
        # print('sep vqvae zup',zup)
        # print(len(zup[0]))
        # pdb.set_trace()
        xup, xup_code = self.vqvae_up.decode_latent(zup)        
        xdown, xdown_code = self.vqvae_down.decode_latent(zdown)
        b, t, cup = xup.size()
        _, _, cdown = xdown.size()
        ...
        _, t_c = xup_code[0].size()
        x_c = torch.zeros(b, 2*t_c).cuda()
        x_c[:, :t_c] = xup_code[0]
        x_c[:, t_c:] = xdown_code[0]
        
        return x.view(b, t, -1), x_c
    
    def return_codebook(self):
        codebook_up = self.vqvae_up.return_codebook()
        codebook_down = self.vqvae_down.return_codebook()
        return codebook_up, codebook_down
        
    def encode(self, x, start_level=0, end_level=None, bs_chunks=1):
        b, t, c = x.size()
        zup , zqup= self.vqvae_up.encode(x.view(b, t, c//self.chanel_num, self.chanel_num)[:, :, smpl_up].view(b, t, -1), start_level, end_level, bs_chunks)
        zdown , zqdown = self.vqvae_down.encode(x.view(b, t, c//self.chanel_num, self.chanel_num)[:, :, smpl_down].view(b, t, -1), start_level, end_level, bs_chunks)
        return (zup, zdown) , (zqup , zqdown)
    ...
    def forward(self, x):
        ...
        print('forward xup',torch.isnan(xup).any())
        ...
        return xout.view(b, t, -1), loss_up, loss_down, [metrics_up, metrics_down]
