...
class VQVAE(nn.Module):
    def __init__(self, hps, input_dim=30):
        super().__init__()
        self.hps = hps
        ...
        print('VQVAE x shape',x_shape)
        print('VQVAE x channels',x_channels)
    ...
    def quantise_latent(self,zs,start_level=0, end_level=None, bs_chunks=1):
        # pdb.set_trace()
        zc = self.bottleneck.encode([zs])
        
        x_outs = self.bottleneck.decode(zc)[start_level:end_level]
    ...
    def _decode_latent(self, zs, start_level=0, end_level=None):
        # Decode
        if end_level is None:
            end_level = self.levels
        
        # xs_quantised, xs_code = self.bottleneck.decode_latent(zs, start_level=start_level, end_level=end_level)
        xs_code = self.bottleneck.encode(zs)
        xs_quantised = self.bottleneck.decode(xs_code)
        # sss = self.bottleneck.encode(zs)
        # pdb.set_trace()
        
        # print('code',sss)
        # print('real_c',xs_code)
        # for i in range(len(xs_code[0])):
        #     if t.all(xs_code[0][i] != sss[0][i]):
        #         print(i,':',xs_code[0][i] != sss[0][i])
        #     else:
        #         print('same')
        # pdb.set_trace()
        assert len(xs_quantised) == end_level - start_level
        # print('xs_quantised laten',xs_quantised[0].size())
        # pdb.set_trace()
        # Use only lowest level
        decoder, x_quantised = self.decoders[start_level], xs_quantised[0:1]
        x_out = decoder(x_quantised, all_levels=False)
        x_out = self.postprocess(x_out)
        # print('vqvae recover',x_out.shape)
        # pdb.set_trace()
        return x_out, xs_code
    '''
    def decode_latent(self, zs, start_level=0, end_level=None, bs_chunks=1):
        # print('zs',zs[0].size())
        z_chunks = [t.chunk(z, bs_chunks, dim=0) for z in zs]
        # print('z_chunk',len(z_chunks))
        # print(z_chunks)
        # print(z_chunks[0][0].size())
        x_outs = []
        for i in range(bs_chunks):
            zs_i = [z_chunk[i] for z_chunk in z_chunks]
            # print('zs_i',len(zs_i))
            ...
            # print(zs_i[0].size())
            # pdb.set_trace()
            x_out, x_code = self._decode_latent(zs_i, start_level=start_level, end_level=end_level)
            x_outs.append(x_out)
        return t.cat(x_outs, dim=0), x_code
    '''
    def decode_latent(self, zs, start_level=0, end_level=None, bs_chunks=1):
        
        x_chunks = [t.chunk(z, bs_chunks, dim=0) for z in zs]
        zc = []
        for x_i in x_chunks:
            c = self.bottleneck.encode(x_i)[start_level:end_level]
            zc.append(c)
        # pdb.set_trace()
        zs = self.bottleneck.encode(zs)[start_level:end_level]    
        zc = [t.cat(zc_level_list, dim=0) for zc_level_list in zip(*zc)]
        zq = self.decode(zc, start_level, end_level, bs_chunks)
        return zq, zc

    def return_codebook(self):
        return self.bottleneck.codebook()
    ...
    def _encode(self, x, start_level=0, end_level=None):
        ...
        # print('encoder preprocess xin',x_in.size())
        xs = []
        ...
        zs = self.bottleneck.encode(xs)  #nearest code
        # print('encode zs',zs[0].size())
        
        zq = self.bottleneck.decode(zs[start_level:end_level], start_level=start_level, end_level=end_level) #nearest latent
       
        # print(zq)
        # xq, xs_code = self.bottleneck.decode_latent(zq, start_level=start_level, end_level=end_level)
        
        # pdb.set_trace()
        # print('code',zs)
        # print('real_c',xs_code)
        # for i in range(len(xs_code[0])):
        #     if t.any(xs_code[0][i] != zs[0][i]):
        #         print(i,':',xs_code[0][i] != zs[0][i])
        #         pdb.set_trace()
        #     else:
        #         print('same')
        ...
        # print('decode zs',zq[0].size())
        return zs[start_level:end_level] , zq[start_level:end_level]
    ...
    def encode(self, x, start_level=0, end_level=None, bs_chunks=1):
        ...
        zq_list = []
        for x_i in x_chunks:
            zs_i, zq_i = self._encode(x_i, start_level=start_level, end_level=end_level)
            zs_list.append(zs_i)
            zq_list.append(zq_i)
        zs = [t.cat(zs_level_list, dim=0) for zs_level_list in zip(*zs_list)]
        zq = [t.cat(zq_level_list, dim=0) for zq_level_list in zip(*zq_list)]
        return zs , zq
    ...
    def forward(self, x):
        ...
        # Encode/Decode
        print('preprocess x',t.isnan(x).any())
        x_in = self.preprocess(x)
        print('preprocess xin',x_in.size())
        print(x_in[0,0,:])
        
        xs = []
        for level in range(self.levels):
            print('encode level',level)
            encoder = self.encoders[level]
            x_out = encoder(x_in)
            print('encoder x out',t.tensor([item.cpu().detach().numpy() for item in x_out]).cuda().size())
            xs.append(x_out[-1])
            print('xs',t.tensor([item.cpu().detach().numpy() for item in x_out]).cuda()[0,:,0])

        zs, xs_quantised, commit_losses, quantiser_metrics = self.bottleneck(xs)
        print('xs quantised:',t.tensor([item.cpu().detach().numpy() for item in xs_quantised]).cuda().size())
        x_outs = []
        for level in range(self.levels):
            ...
            print('decoder x out',t.tensor([item.cpu().detach().numpy() for item in x_out]).cuda().size())
            print(x_out)
        ...
        if x.shape[-1]==16:
            #计算脖子loss
            neck_loss = t.zeros(()).to(x.device)
            x_neck=t.mean(x_target[:,:,0:2]-x_target[:,:,-2:])
            for level in reversed(range(self.levels)):
                x_out = self.postprocess(x_outs[level])
                neck_loss += _loss_fn(x_neck, x_out[:,:,0]-x_out[:,:,13])
                print('neck_loss',neck_loss)
            loss += neck_loss
        print('loss',loss)
        # pdb.set_trace()
