...
def visualizeAndWrite(results,config,expdir,dance_names, epoch, quants=None):
    ...
    if config.data.name == "aist":
        ...
        for i in range(len(results)):
            ...
            dance_datas.append(np_dance)
            # root = np_dance[:, -1]
            # np_dance = np_dance + np.tile(root, (1, 15))
            # np_dance[:, -1] = root
            b, c = np_dance.shape
            print('func dance shape',np_dance.shape)
            # np_dance_re = np_dance.reshape([b, c//2, 2])
            # np_dacne_new=np_dance_re[:,:]+(300,500)
            # print('add dance',np_dacne_new)
            #pdb.set_trace()
            np_dacne_new = np_dance.reshape([b,-1])
            np_dances.append(np_dacne_new)
...
def load_data(data_dir, interval=900, data_type='2D'):
    ...
    for fnames in fnames:
        path = os.path.join(data_dir, fnames)
        with open(path) as f:
...
def load_data_pkl(data_dir, interval=900, data_type='2D'):
    music_data, dance_data = [], []
    #fnames = sorted(os.listdir(data_dir))
    # print(fnames)
    fnames_single = sorted(os.listdir(data_dir+'/single_keypoints_2d/'))
    fnames_multi=sorted(os.listdir(data_dir+'/multi_keypoints_2d/'))
    
    if ".ipynb_checkpoints" in fnames_single:
        fnames_single.remove(".ipynb_checkpoints")
        
    if ".ipynb_checkpoints" in fnames_multi:
        fnames_multi.remove(".ipynb_checkpoints")
        
    print('load single data')
    for fname in fnames_single:
        path = data_dir+'/single_keypoints_2d/'+fname
        np_dance = np.load(path,allow_pickle=True)
        print('dance',True in np.isnan(np_dance))
        seq_len, dim, _ = np_dance.shape
        np_dance=np_dance.reshape(seq_len,-1)
        print('dance reshape',np_dance.shape)
        print(True in np.isnan(np_dance))
    ...
    for fnamem in fnames_multi:
        path = data_dir+'/multi_keypoints_2d/'+fnamem
        fpeople=sorted(os.listdir(path))
        for fp in fpeople:
            np_dance = np.load(path+'/'+fp,allow_pickle=True)
            seq_len, dim, _ = np_dance.shape
            np_dance=np_dance.reshape(seq_len,-1)
...
def load_data_aist(data_dir, interval=120, move=40, rotmat=False, external_wav=None, external_wav_rate=1, music_normalize=False, wav_padding=0):
    ...
    for fname in fnames:
        ...
        with open(path) as f:
            ...
            if interval is not None:
                seq_len, dim = np_music.shape
                #print('music seq len:',seq_len)
                #print('dance seq len:',np_dance.shape[0])
...
def load_test_data_pkl(data_dir, data_type='2D',down_rate=1):
    tot = 0
    music_data, dance_data = [], []
    fnames = sorted(os.listdir(data_dir))
    #print(fnames)
    fnames = fnames[1405:]  # For debug
    for fname in fnames:
        path = os.path.join(data_dir, fname)
        np_dance = np.load(path,allow_pickle=True)
        lenght=len(np_dance)
        n=lenght // down_rate
        np_dances=np_dance[:n*down_rate]
        np_dances=np_dances.reshape(np_dances.shape[0],-1)
        #music_data.append(np_music)
        print('test dances shape', np_dances.shape)
        #pdb.set_trace()
        dance_data.append(np_dances)
