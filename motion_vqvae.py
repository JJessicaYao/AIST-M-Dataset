...
import pickle
# from dataset_convert.motion_seq import MoSeq, paired_collate_fn
...
import itertools
import numpy as np
from vqvae.sep_vqvae import SepVQVAE
import datetime
...
class MoQ():
    ...
    def train(self):
        ...
        writer=SummaryWriter(self.expdir)
        # print(self.expdir)
        # pdb.set_trace()
        
        # inp_tensor=Variable(torch.randn(size=(1,400,30)))
        # out=model(inp_tensor)
        # graph=make_dot(out[0],params=dict(model.named_parameters()),)
        # graph.render(filename='vqvae',view=False,format='png')
        # pdb.set_trace()
        ...
        for epoch_i in range(1, config.epoch + 1):
            ...
            for batch_i, batch in enumerate(training_data):
                ...
                #     
                #     writer.add_graph(model, pose_seq)
                trans = None
                pose_seq = batch.to(self.device)
                print(config.rotmat)
                print('pose sequnce shape',np.array(batch.cpu()).shape)
                print(torch.isnan(pose_seq).any())
                #pdb.set_trace()
                ...
                # print('finally size:',pose_seq.size())
                optimizer.zero_grad()

                output, loss_u,loss_d, metrics = model(pose_seq)

                loss=(loss_u+2*loss_d)*0.5
                ...
                if batch_i % 100 == 0:
                    writer.add_scalar('loss',loss,epoch_i+int(batch_i/100))
                    writer.add_scalar('loss up',loss_u,epoch_i+int(batch_i/100))
                    writer.add_scalar('loss down',loss_d,epoch_i+int(batch_i/100))
            ...
            if epoch_i % config.test_freq == 0:
                with torch.no_grad():
                    ...
                    for i_eval, batch_eval in enumerate(tqdm(test_loader, desc='Generating Dance Poses')):
                        ...
                        global_shift = src_pos_eval[:, :, :2].clone()
                        # if config.rotmat:
                        #     # trans = pose_seq[:, :, :3]
                        ...
                        pose_seq_out, loss_u, loss_d, _ = model(src_pos_eval)  # first 20 secs
                        #print('train',summary(model,src_pos_eval))
                        

                        if config.rotmat:
                            pose_seq_out = torch.cat([global_shift, pose_seq_out], dim=2)
                        if config.global_vel:
                            global_vel = pose_seq_out[:, :, :2].clone()
                            pose_seq_out[:, 0, :2] = 0
                            for iii in range(1, pose_seq_out.size(1)):
                                pose_seq_out[:, iii, :2] = pose_seq_out[:, iii-1, :2] + global_vel[:, iii-1, :]
                            # print('Use vel!')
                            # print(pose_seq_out[:, :, :3])
                        else:
                            pose_seq_out[:, :, :2] = global_shift
    ...
    def eval(self):
        with torch.no_grad():
            ...
            for i_eval, batch_eval in enumerate(tqdm(self.test_loader, desc='Generating Dance Poses')):
                ...
                pose_seq_out, loss_u,loss_d, _ = model(src_pos_eval)
    ...
    def analyze_code(self,):
        ...
        for i_eval, batch_eval in enumerate(tqdm(self.training_data, desc='Generating Dance Poses')):
            ...
            quants = model.module.encode(pose_seq_eval)[0][0].cpu().data.numpy()
    ...
    def convert(self):
        with torch.no_grad():
            ...
            self.device = torch.device('cuda' if config.cuda else 'cpu')
            print("Evaluation...")
            checkpoint = torch.load(self.config.vqvae_weight)
            self.model.load_state_dict(checkpoint['model'])
            self.model.eval()
            ...
            path='./diff_vqvae/dev_net/'
            pfile=os.listdir(path)
            for f in pfile:
                data=np.load(path+f,allow_pickle=True)
                pred= torch.from_numpy(data['dance_results']).to(self.device)
                gt = torch.from_numpy(data['ground_truth']).to(self.device)
                # pdb.set_trace()
                _, t = pred.size()
                pose_decode, pose_c = model.module.decode_latent(([pred[:, :t//2].transpose(1,0).reshape(-1,512,1)],[pred[:, t//2:].transpose(1,0).reshape(-1,512,1)])) 
                gt_decode, gt_c = model.module.decode_latent(([gt[:, :t//2].transpose(1,0).reshape(-1,512,1)],[gt[:, t//2:].transpose(1,0).reshape(-1,512,1)])) 

                sp='./diff_vqvae/convert/sim'+str(data['similarity'])+'/'
                if not os.path.exists(sp):
                    os.makedirs(sp)

                with open(sp+f,'wb') as save:
                    dict={'pred':pose_decode,'gt':gt_decode}
    ...
    def _build_train_loader(self):
        ...
        elif data.name == "s_aist":
            print('use own data set!')
            print("train_dir",data.train_dir)
            train_music_data, train_dance_data = load_data_pkl(
                data.train_dir,
                interval=data.seq_len,
                data_type=data.data_type
            )
            print(True in np.isnan(train_dance_data))
        else:
    ...
    def _build_test_loader(self):
        ...
        if data.name == "aist":
            ...
            print ("test with own dataset!")
            music_data, dance_data, dance_names = load_test_data_pkl(
                data.test_dir, data_type=data.data_type, down_rate=config.ds_rate)
